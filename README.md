...
-React Project Structure, React Project Flow
-JSX , Expressions in JSX
-Class Components
...
3. Go to the crated Project, install , and serve the application
	cd my-app
	npm install
	npm run dev
...
-It supports unidirectional data flow (data flows from parent to child components).
-React uses JSX (JavaScript XML) to write UI in a syntax similar to HTML inside JavaScript.
...
-The difference between a library and a framework is that 
-a library only helps us in one aspect whereas a framework helps us in many aspects. 
...
React Features
--------------
-Component-based architecture (reusable, maintainable, loosely coupled)
...
Why Virtual DOM?
================
-Frequent direct DOM manipulations are expensive and can impact performance.
-Every time the real DOM changes, the browser may need to:
    • Recalculate CSS
    • Perform layout (reflow)
    • Repaint the screen
-Updating the real DOM repeatedly is slower compared to updating JavaScript objects in memory.
-To improve performance, React minimizes direct manipulation of the real DOM using the Virtual DOM.
...
-It is a tree of JavaScript objects representing UI elements, their attributes, and content.
...
-After identifying the changes, React updates only the required parts of the real DOM.
-This process is called Reconciliation.
-The Virtual DOM helps React optimize rendering performance.
...
2. Optimized Updates
   - React updates only the changed elements instead of re-rendering the entire page.
3. Faster Rendering
   - Minimizes browser reflow and repaint operations.
4. Simplified UI Development
   - Developers work with declarative UI instead of manually manipulating the DOM.
...
==================================
1. Initial Render : When the application loads, React creates a Virtual DOM tree based on the JSX components.

2. State or Props Change : Whenever state or props change (such as user input or API response), React creates a new Virtual DOM tree with the updated data.

3. Diffing : React compares the new Virtual DOM with the previous Virtual DOM tree to identify what has changed.

4. Reconciliation : Based on the diffing result, React determines the minimum changes required in the real DOM.

5. Real DOM Update : React updates only the changed parts of the real DOM instead of re-rendering the entire page, resulting in faster UI updates.
...
Shadow DOM
----------
-Shadow DOM is a browser technology used in Web Components.
-It provides encapsulation for HTML structure and CSS styles.
-Styles inside the Shadow DOM do not affect the outside DOM.
-Mainly used for component isolation.

Virtual DOM
-----------
-Virtual DOM is a programming concept used by React.
-It is a lightweight JavaScript representation of the real DOM.
-React uses the Virtual DOM to optimize UI rendering and improve performance.
-It helps minimize direct DOM manipulations.

Key Difference
--------------
-Shadow DOM focuses on encapsulation and component isolation.
-Virtual DOM focuses on efficient UI updates and rendering performance.
...
README.md : Contains introductory documentation and setup instructions for the application.
package.json : Contains project metadata, npm dependencies, scripts, and configuration details.
package-lock.json : Stores the exact dependency tree and package versions installed by npm.
node_modules : Contains all installed npm packages and project dependencies.
public : Contains static files such as images, icons, and HTML files that are served directly.
src : Contains the main source code of the React application such as components, styles, services, and utilities.
...
1. index.html
   ----------
   Contains the root container element:
   <div id="root"></div>
   React injects the application into this container.
...
-render() renders the App component into the root container.
...
React.StrictMode
================
-StrictMode is a React development tool used for identifying potential problems in an application.
-It enables additional checks and warnings for its descendant components.
-StrictMode checks run only in development mode and do not affect the production build.
-StrictMode helps with the following:
    • Identifying unsafe lifecycle methods
    • Warning about legacy string ref API usage
    • Warning about deprecated findDOMNode() usage
    • Detecting unexpected side effects
    • Detecting legacy context API usage
    • Detecting components with non-reusable state
	
-ex: import { StrictMode } from 'react'
     <StrictMode>
		<App />
     </StrictMode>
...
-React components split the UI into independent, reusable pieces, and think about each piece in isolation. 
...
2. add below 2 imports in main.jsx file
...
===========================================================
-Bootstrap can be used in a react application just by adding the below 2 CDN links in 'index.html'
...
2. add below import in main.jsx file
   import 'bootstrap-icons/font/bootstrap-icons.css';

3. use classnames in JSX
...
Conditional CSS 
================
-Conditional CSS means applying styles dynamically based on conditions.
-Conditional CSS is commonly used for:
    • Theme switching
    • Form validation
...
1. Conditional Inline Style : <h1 style={{ color: isActive ? "green" : "red" }}>Status</h1>

2. Conditional className : <h1 className={success ? "success" : "error"}>Message</h1>

3. Conditional Background Color : <div style={{ backgroundColor: darkMode ? "black" : "white" }}>Theme</div>

4. Conditional Font Size : <p style={{ fontSize: large ? "30px" : "16px" }}>React Text</p>

5. Conditional Visibility : <div style={{ display: show ? "block" : "none" }}>Content</div>

6. Conditional Border Color : <input style={{ border: error ? "2px solid red" : "2px solid green" }} />

7. Conditional Button Disable : <button disabled={name === ""}>Submit</button>

8. Multiple Conditional Classes : <div className={isActive ? "box active" : "box"}>Card</div>

9. Conditional Opacity : <img style={{ opacity: disabled ? 0.5 : 1 }} src="image.png" />

10. Conditional Cursor : <button style={{ cursor: loading ? "not-allowed" : "pointer" }}>Save</button>
...
-React primarily uses one-way data binding. 
-Two-way binding can be implemented manually using state, value, and onChange handlers.
...
-It is commonly used to show, hide, or switch UI content dynamically.
-React uses JavaScript conditions to control what gets rendered on the screen.


Use Cases
=========
-Rendering data received from APIs
-Showing or hiding elements
-Toggling application functionality
-Implementing role-based access or permission levels
...
4. Using Logical && Operator (Short Circuit Evaluation)

Examples
========
Ternary Operator :  <div>{flag ? <h1>Hello</h1> : null}</div>
Short Circuit Evaluation:  <div>{flag && <h1>Hello</h1>}</div>
...
Lists and Keys
==============
-Lists are used to display multiple items dynamically.
-Arrays are commonly rendered using the map() method.
-Each item in the list is converted into a JSX element.
...
-A 'key' is a special attribute that should be provided while rendering list items in React.
-React displays a warning in the console if the key prop is missing for list items.
-Keys provide a stable and unique identity to elements.
-Keys help React identify:
    • Which items changed
    • Which items were added
    • Which items were removed
-React uses keys during the reconciliation process to optimize rendering performance.
-During the initial render, React creates DOM nodes for all list items.
-On subsequent renders, React compares the previous list with the new list using keys and updates only the modified elements instead of recreating the entire list.


Tip
===
-Avoid using the array index as a key when:
    • The list can be filtered
    • The list can be sorted
    • Items can be added or removed dynamically
-In such cases, index values may change, causing React to treat items as different elements and unnecessarily re-render the list.


Why map() is used instead of forEach()
======================================
-map() iterates through each element of the array. For every item, it returns a JSX element. 
-React collects all returned JSX elements and renders them on the screen.
-forEach() does not return anything (returns undefined), so it cannot be used directly inside JSX..
 
 
 
Read data from a JSON file (https://jsonplaceholder.typicode.com/users)
==========================
1. create a JSON file add data (user_data.json) 
   [ {},{},{} ]
   
2. import userArr from './user_data.json'

3. use userArr(List data) in JSX
			{userArr.map((user)=>{
                return <tr>
                    <td>{user.id}</td>
                    <td>{user.name}</td>                    
...
Read data from a Javascript file (https://fakestoreapi.com/products)
================================
1. create a javascript file and export data 
   export default [ {},{},{} ]
   
2. import productArr from './product_data.ts'

3. use productArr(List data) in JSX
...
Assignment
==========
1.Display List of employees in a table
  display a dropdown with 'all' , 'male' & 'female' 
  display the employees based on the dropdown value selected
...
2. Create an array of task objects, and display them using map()
...
forceUpdate()
=============
-React components normally re-render when there are changes to state or props.
-Updating normal class variables or function variables does not trigger a re-render.
-In rare cases, forceUpdate() can be used to force a component to re-render.
-forceUpdate() is available only in class components.

Example:
---------
this.forceUpdate();
-React recommends avoiding forceUpdate() in most situations.
-Instead of using forceUpdate(), state updates should be preferred.
-Common use cases for non-state variables:
    • Storing timeout/interval IDs
    • Storing frequently used values
    • Referencing DOM elements
    • Mutable values that do not affect UI
-Normally, render() should depend only on:
    • this.props
    • this.state

-Functional components do not have a forceUpdate() method.
-In functional components, re-rendering is triggered using:
    • useState()
    • useReducer()
-useRef() can store mutable values without causing re-renders.
...
Props
=====
-Props stands for Properties. Props are inputs to components.
-Props are used to pass data and methods from a parent component to a child component.
-Attributes passed to components are received as an object called props.
-Props help make components reusable and dynamic.
-Data passed using props flows in one direction (Parent → Child). This is called unidirectional data flow.
-We can pass props to components similar to HTML attributes.

Example:
---------
<ChildComponent name="John" age={25} />
-Boolean props passed without a value are treated as true.
	<MyComponent showTitle={true} />
	<MyComponent showTitle />

-Props can be accessed as shown below:
	Class Component: this.props.propName
	Functional Component: props.propName


Important Points
================
1. Props are immutable.
   -Child components should not modify props received from parent components.

2. Props support unidirectional data flow.
   -Data flows only from Parent → Child.

3. Props help create reusable components.

4. When parent data changes, updated props are automatically passed to child components.
...
-It helps avoid repeatedly writing props or this.props.
...
PropTypes
=========
-React supports runtime prop validation using the prop-types package.
-PropTypes help validate the data received by a component.
-Warnings are shown in the browser console if invalid prop types are passed.
-PropTypes are checked only in development mode.
...
Requiring Props
===============
-If a prop is not passed to a component, its value becomes undefined.
-isRequired ensures that a prop must be provided.
...
Default Props
=============
-Default props allow setting default values for props.
...
Props.children
==============
-props.children represents the content placed between the opening and closing tags of a component.
-props.children can contain text, JSX, single elements, or multiple elements.
-props.children is available in every component.
...
=====================
-PropTypes.element ensures that only one child element is passed to a component.
-Passing multiple child elements will show a warning.
...
==========================
-Functions can also be passed as props from parent to child components.
-This helps child components communicate with parent components.

Example:
---------
<Child onDelete={deleteUser} />



Props Drilling
==============
-Props drilling means passing props through multiple intermediate components to reach deeply nested child components.
-It can make component structures difficult to maintain.
-Context API/REDUX is commonly used to avoid props drilling.
...
3. on button click call a function & write sweetalert code	
...
SyntheticEvent
==============
-Instead of working directly with the browser's native event system(MouseEvent, KeyboardEvent, PointerEvent, etc.), 
 React provides its own event system called SyntheticEvent.
-A SyntheticEvent is a cross-browser wrapper around the browser's native event.
-It normalizes browser differences, ensuring that events behave consistently across different browsers(Chrome, Firefox, Safari)
-In React, event handler functions receive a SyntheticEvent object as an argument instead of the browser's native Event object.
-SyntheticEvent provides the same interface as native events, including properties and methods such as:
    * target
    * currentTarget
    * preventDefault()
    * stopPropagation()	
-If access to the underlying native browser event is required, it can be obtained using: event.nativeEvent
...
2. Consistent event behavior across all supported browsers.
3. Improved performance through React's event delegation mechanism.
4. Simplifies event handling for developers.
...
-PureComponent checks previous State and new State , if nothing changed , it skips re-rendering
...
-It is used with child components that receive props.
-When a component is wrapped with React.memo(), React re-renders it only when its props change.
-React.memo() performs a shallow comparison of props. If the props are unchanged, React reuses the previous rendered result.
-Avoids unnecessary re-rendering when parent components re-render.
...
-It helps avoid unnecessary re-calculations during re-renders.
-useMemo() returns a memoized value.
-The function passed to useMemo() executes only when one of its dependencies changes.
-If the dependency array is empty ([]), the calculation runs only on the initial render.
-useMemo() should be used for expensive calculations such as filtering, sorting, or complex computations.
-useMemo() returns a memoized value, whereas useCallback() returns a memoized function.
...
const filteredProducts = useMemo(() => {
    ...
    return products.filter(product => product.toLowerCase().includes(searchText.toLowerCase()));
  }, [searchText]);
...
-It helps prevent unnecessary re-creation of functions during component re-renders.
-It is used when passing functions as props to child components.
-It preserves the same function reference between parent renders unless its dependencies change.

-Without useCallback(), a new function object is created every time the parent component renders.
-When this function is passed as a prop to a child component, the child may re-render because the prop reference has changed
-This can cause child components to re-render unnecessarily(even if the child is using React.memo())
...
LifeCycle hooks in Functional Component
========================================
-Functional Components don't have lifecycle methods like class components.
-lifecycle behaviour is handled with hooks , primarily useEffect()



useEffect()
----------
-useEffect() is a React hook that allows to execute some code after the component is initially rendered & when state change happens.
-Common Functionalities : fetching data from API , Updating DOM , timers , event listeners
-fetching user data when selected user Id changes

-useEffect() takes a callback function as the 1st argument and a dependency array as the 2nd argument.
  Example:
  useEffect(() => {}, [])

-If the 2nd argument is omitted, the effect runs after every render (state and prop changes).
  useEffect(() => {})
  
-When the dependency array is empty ([]), the effect runs only once after the initial render 
 the effect will not re-run when state value changes
 useEffect(() => {}, [])

-If dependencies are provided, the effect runs whenever any dependency value changes.
 useEffect(() => {}, [a,b])
  
-We can have multiple useEffect() hooks in the same component to handle different side effects.

-To perform cleanup (similar to componentWillUnmount), return a function from useEffect().
  Example:
  useEffect(() => {
...
==================================
-Refs are mainly used when we need to work with the DOM directly instead of using React state.
...
----------------------------------------------
 ...
 -Used for UI data     -Used for DOM access or mutable values 
...
Fetch API 
=========
-Built into modern browsers and Node.js (no installation required).
-Request body usually needs JSON.stringify().
-Requires response.json() to parse JSON responses.
-Supports request cancellation via AbortController, but no built-in timeout.
-No native support for interceptors.


Axios
=====
-Requires installation (npm install axios).
-Automatically serializes request data and parses JSON responses.
-Supports request cancellation and built-in timeout configuration.
-Supports request and response interceptors.
-Automatically rejects promises for HTTP errors (4xx/5xx), simplifying error handling                                                       
...
Making multiple requests with axios.all() 
========================================
-Executes multiple requests in parallel, not sequentially.
-Similar to Javascript's Promise.all()
-Total execution time depends on the slowest request.
-If one request fails, Promise.all()/axios.all() fails immediately.
...
Http Interceptors
=================
-Interceptors allow us to intercept/modify HTTP requests or responses before they are sent to the server or before they are processed by then() / catch().
 
 Client Sends httpRequest ----> Interceptor ----> ModifiedRequest ----> Backend Server
 Client receives ModifiedResponse<---- Interceptor <---- httpResponse <---- Backend Server
...
-Avoid duplicate code across API calls.
 
 
 

Request interceptor use cases:
-Validate the presence of authentication tokens before sending requests.
-Redirect users to login if authentication information is missing.
...
Backend Server --> response --> interceptor --> modifiedResponse --> component
...
====================================
-Higher Order Components  are used for reusing component logic across multiple components without duplicating code.
-A Higher Order Component is a JavaScript function that accepts a component as an argument and returns a new enhanced component.
	EnhancedComponent = higherOrderComponent(OriginalComponent)
-HOCs do not modify the original component. Instead, they wrap the originalcomponent and provide additional functionality.
-Benefits:	1. Code Reusability, 2.Centralized Logic

-HOCs are generally used for:
...
export default MyCounter;  				// without Logging
export default withLogger(MyCounter);   // with Logging
...
Routing
=======
-Single Page Applications (SPAs) load a single HTML page and dynamically update content without reloading the entire page.
-Only the required portion of the page is updated, resulting in a faster and smoother user experience.
-Routing enables navigation between different views or components within an SPA.
-When a user clicks a link or the URL changes, React Router renders the appropriate component without refreshing the page.
-Routing keeps the browser URL synchronized with the application's current view.
-Browser Back and Forward buttons work seamlessly, just like in traditional websites.
-It connects application navigation with browser navigation features such as the address bar, history stack, and navigation buttons.
-React Router allows us to display specific components based on the current URL (route) that matches the requested path.
...
2. Create Pages/Components
   create components like - (Home,About,Contact,Careers,NotFound)   
3. set up the application to work with React Router. (index.js / main.tsx)
...
Link vs NavLink
===============
<Link> does not automatically apply any styling to the currently active route.
<NavLink> is similar to <Link>, but it automatically adds an 'active' class to the link that matches the current URL.
-This makes it easy to highlight the currently selected navigation item.
ex: <NavLink to='home' className='nav-link'>Home</NavLink>

-add below css: 
	nav .active{background-color: aquamarine;}



Navigate from one Route to another Route
=========================================
-Using <a> tag is not recommended because it refreshes the entire application.
	<a href="/home">Go To Home</a>

-Using <Link> is recommended because it navigates without reloading the page.
...
2. Create navigate object
   const navigate = useNavigate();
   
3. on user action call a function, that function should call navigate()
...
2. create a link to that route (UserList - for Every User)
   <Link to={`/userdetails/${id}`}>Details</Link>
   
3. Read the Parameter data and display
   const { id } = useParams();
...
2. create a link & navigation logic (ProductList page)
  const navigate = useNavigate();
...
Note: To get all the query params as object, below code can be used
      productObj = Object.fromEntries([...searchParams]);
  
  
Nested Routing
==============
-nested routing is used so that a parent component has control over its child component at the route level.
-Route: Careers (nested Routes: PermanentJobs, ContractJobs)
		<Route path="/careers" element={<Careers />}>
            <Route path="/careers/permanent" element={<PermanentJobs />} />
            <Route path="/careers/contract" element={<ContractJobs />} />
        </Route>
-add a nav in Parent(Careers) component, and a outlet	
     <div>
...
Index Route
===========
-an index route is a special child route that renders by default when parent route loads/matches.
<Route path="/careers" element={<Careers />}>
...
===========================
-Eager Loading: Components are loaded when the application starts.
-Lazy Loading: Components are loaded only when they are required.

-Faster initial page load, Loads components only when needed
-React.lazy() is used to load a component dynamically.

  Eager Import: import OtherComponent from './OtherComponent';
  Lazy Import : const OtherComponent = React.lazy(() => import('./OtherComponent'));

Suspense
--------
-A lazy-loaded component must be wrapped inside Suspense.
...
ESLint
======
-ESLint is a static code analysis tool for identifying problematic code present in JavaScript/Typescript code.
-It Examines and list down the errors , bad practices, style issues present in our project.


1. npm i eslint -g
2. npm init @eslint/config@latest --force
         (OR) 
   npx eslint --init
...
End of Complete React Documentation.
